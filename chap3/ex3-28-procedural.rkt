...
(define proc
  (lambda (vars body)
    (lambda (sym)
      (cond [(eq? sym 'proc?) #t]
            [(eq? sym 'proc-var) vars]
            [(eq? sym 'proc-body) body]
            [else (eopl:error 'proc "invalid arg: ~s" sym)]))))
...
(define proc-body
  (lambda (p) (p 'proc-body)))

(define apply-proc
  (lambda (p vals env)
    (let ([val (value-of (car vals) env)])
      (let ([new-body
             (value-of (replace-ocurrence
                        (car (proc-var p)) val (proc-body p)) env)])
        (if (null? (cdr vals))
            new-body
  ...
  (lambda (var val body)
    (cases expression body
           ...
           (if-exp (exp1 exp2 exp3)
                   (if-exp (replace-ocurrence-bool var val exp1)
                           (replace-ocurrence var val exp2)
           ...
           (call-exp (rator rand)
                     (call-exp (replace-ocurrence var val rator)
                               (map (lambda (x)
                                      (replace-ocurrence var val x))
                                    rand))))))

(define replace-ocurrence-bool
  (lambda (var val exp1)
    (cases bool-expr exp1
           (zero?-exp (exp1)
                      (zero?-exp (replace-ocurrence var val exp1)))
           (equal?-exp (exp1 exp2)
                       (equal?-exp (replace-ocurrence var val exp1)
                                   (replace-ocurrence var val exp2)))
           (greater?-exp (exp1 exp2)
                         (greater?-exp (replace-ocurrence var val exp1)
                                       (replace-ocurrence var val exp2)))
           (less?-exp (exp1 exp2)
                      (less?-exp (replace-ocurrence var val exp1)
                                 (replace-ocurrence var val exp2))))))
