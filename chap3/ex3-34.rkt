  ...
  (lambda ()
    (lambda (sym)
      (cond [(eq? sym 'empty-env?) #t]
            [(eq? sym 'environment?) #t]
            [(eq? sym 'apply-env)
             (lambda (search-var)
               (report-no-binding-found search-var))]
            [else (report-invalid-arg sym)]))))
...
(define extend-env-rec
  (lambda (p-name b-var p-body env)
    (lambda (sym)
      (cond [(eq? sym 'empty-env?)]
            [(eq? sym 'environment?) #t]
            [(eq? sym 'apply-env)
             (lambda (search-var)
               (if (eq? p-name search-var)
                   (proc-val (proc b-var p-body
                                   (extend-env-rec p-name b-var p-body env)))
                   (apply-env env search-var)))]
            [else (report-invalid-arg sym)]))))

(define environment?
  (lambda (env) (env 'environment?)))
(define empty-env?
  (lambda (env) (env 'empty-env?)))
(define apply-env
  (lambda (env search-var) ((env 'apply-env) search-var)))
