...
(define report-invalid-reference
  (lambda (ref store)
    (eopl:error 'store "invalid-reference: ~s, ~s" ref store)))
...
(define value-of-program
  (lambda (pgm)
    ...
    (cases program pgm
           (a-program
            (exp1)
            (let ([result (value-of exp1 (init-env) (get-store))])
              (cases answer result
...
;; ExpVal = Int + Bool + List + Proc + Ref(ExpVal)
...
(define value-of
  (lambda (exp env store)
    (cases expression exp
           ...
           (diff-exp (exp1 exp2) (eval-arith exp1 exp2 - env store))
           (if-exp (exp1 exp2 exp3) (eval-if exp1 exp2 exp3 env))
           (let-exp (var exp1 body) (eval-let var exp1 body env store))
           ...
           (newref-exp (exp1) (eval-newref exp1 env store))
           (deref-exp (exp1) (eval-deref exp1 env store))
           (setref-exp (exp1 exp2) (eval-setref exp1 exp2 env store))
           (begin-exp (exp1 exps) (eval-begin exp1 exps env))
           )))
...
(define eval-arith
  (lambda (exp1 exp2 f env store)
    (let ([ans1 (value-of exp1 env store)])
      (cases answer ans1
             (an-answer
              (v1 s1)
              (let ([ans2 (value-of exp2 env s1)])
                (cases answer ans2
                       (an-answer
                        (v2 s2)
                        (let ([result (num-val
...
(define eval-let
  (lambda (var exp1 body env store)
    (let ([res1 (value-of exp1 env store)])
      (cases answer res1
             (an-answer
              (v1 s1)
              (let ([new-env (extend-env var v1 env)])
                (let ([res2 (value-of body new-env s1)])
                  res2
                  ;; (cases answer res2
                  ;;        (an-answer
                  ;;         (v2 s2)
...
(define eval-newref
  (lambda (exp1 env store)
    (let ([res (value-of exp1 env store)])
      (cases answer res
             (an-answer (v1 s1)
                        (an-answer v1 (ref-val (newref v1))))))))
      ...
      (cases answer res1
             (an-answer
              ...
              (let ([ref1 (expval->ref v1)])
                (an-answer (deref ref1) s1)))))))

(define eval-setref
  (lambda (exp1 exp2 env store)
    (let ([res1 (value-of exp1 env store)])
      (cases answer res1
             (an-answer
              (v1 s1)
              (let ([ref (expval->ref v1)])
                (let ([res2 (value-of exp2 env s1)])
                  (cases answer res2
                         (an-answer
                          (v2 s2)
                          (begin
                            (setref! ref v2)
                            (an-answer (expval->num v2) s2)))))))))))

(define eval-begin
  (lambda (exp1 exps env store)
    (let ([res1 (value-of exp1 env store)])
      (cases
       answer res1
       (an-answer
        (v1 s1)
        (letrec
            ([loop (lambda (lst acc s)
                     (if (null? lst)
                         acc
                         (let ([res2 (value-of (car lst) env s1)])
                           (cases
                            answer res2
                            (an-answer
                             (v2 s2)
  ...
  '((program (expression) a-program)
    ...
    (expression
     ("letrec*"
      ...
      "in" expression) letrec*-exp)
    (expression
     ("newref" "(" expression ")") newref-exp)
    (expression
     ("deref" "(" expression ")") deref-exp)
    (expression
     ("setref" "(" expression "," expression ")") setref-exp)
    (expression
